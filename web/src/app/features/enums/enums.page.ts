...
import type { EnumListItem } from '../../core/models/metadata.models';
...
export class EnumsPage {
  ...
  protected readonly items = signal<EnumListItem[]>([]);
  ...
  private loadEnums(q = '') {
    ...
    return this.api
      ...
      .pipe(
        catchError((err) => {
          this.snack.error(`加载枚举值失败：${formatApiError(err)}`);
          return of({ items: [] as EnumListItem[], total: 0, page: 1, page_size: this.pageSize() });
        }),
        switchMap((page) => {
